...
exports.uploadTrailer = async (req, res) => {
  const { file } = req;
  if (!file) return sendError(res, "Video bulunamadı!");
...
exports.createMovie = async (req, res) => {
  ...
  if (director) {
    if (!isValidObjectId(director))
      return sendError(res, "Geçersiz yönetmen kimliği!");
  ...
  if (writers) {
    for (let writerId of writers) {
      if (!isValidObjectId(writerId))
        return sendError(res, "Geçersiz senarist kimliği!");
    }
...
exports.updateMovieWithoutPoster = async (req, res) => {
  ...
  if (!isValidObjectId(movieId)) return sendError(res, "Geçersiz film ID'si!");
  ...
  res.json({ message: "Film güncellendi!", movie });
};
...
exports.updateMovie = async (req, res) => {
  ...
  if (!isValidObjectId(movieId)) return sendError(res, "Geçersiz film kimliği!");

  // if (!req.file) return sendError(res, "Film afişi bulunumadı!");
  ...
  if (file) {
    ...
    if (posterID) {
      ...
      if (result !== "ok") {
        return sendError(res, "Afiş güncellenemedi!");
      }
...
exports.removeMovie = async (req, res) => {
  ...
  // poster olup olmadığını kontrol edin.
  // eğer varsa o zaman onu kaldırmamız gerekir.

  const posterId = movie.poster?.public_id;
  if (posterId) {
    const { result } = await cloudinary.uploader.destroy(posterId);
    if (result !== "ok") return sendError(res, "Afiş bulunamadı!");
  }

  // removing trailer
  const trailerId = movie.trailer?.public_id;
  if (!trailerId) return sendError(res, "Fragman bulunamadı!");
  const { result } = await cloudinary.uploader.destroy(trailerId, {
    resource_type: "video",
  });
  if (result !== "ok") return sendError(res, "Fragman bulunamadı!");
...
exports.getMovieForUpdate = async (req, res) => {
  ...
  if (!isValidObjectId(movieId)) return sendError(res, "ID geçersiz!");
...
exports.searchMovies = async (req, res) => {
  ...
  if (!title.trim()) return sendError(res, "Geçersiz istek!");
